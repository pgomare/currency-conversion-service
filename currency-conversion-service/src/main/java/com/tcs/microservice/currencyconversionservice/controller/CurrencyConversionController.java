...
public class CurrencyConversionController {
	 ...
	 * from, @PathVariable String to, @PathVariable BigDecimal quantity) { return
	 * new CurrencyConversionBean(1L, from,to,BigDecimal.TEN, quantity,quantity); }
	 */

	@GetMapping("/currency-converter/from/{from}/to/{to}/quantity/{quantity}") // where {from} and {to} represents the
																				// column return a bean back
	public CurrencyConversionBean convertCurrency(@PathVariable String from, @PathVariable String to,
		...
		ResponseEntity<CurrencyConversionBean> responseEntity = new RestTemplate().getForEntity(
				"http://localhost:9021/currency-exchange/from/{from}/to/{to}", CurrencyConversionBean.class,
				uriVariable);
	...
	//callin same http://localhost:9021/currency-exchange/from/{from}/to/{to} service by udsin RIbbon and Feign
	@GetMapping("/currency-converter-feign/from/{from}/to/{to}/quantity/{quantity}") // where {from} and {to} represents
																						// the column return a bean back
